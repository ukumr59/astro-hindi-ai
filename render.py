...
def prepare_deities():
    """
    Use only the bundled HD devotional artwork.

    This intentionally performs NO network requests. The previous renderer
    tried Wikimedia/Museum image downloads and failed with 403/404/429 errors.
    All seven deity images are shipped inside assets/deities at 1600x1600.
    """
    required = {
        "Hanuman": DEITY_ASSETS / "hanuman.jpg",
        "Lakshmi goddess": DEITY_ASSETS / "lakshmi.jpg",
        "Ganesha": DEITY_ASSETS / "ganesha.jpg",
        "Shiva Hindu god": DEITY_ASSETS / "shiva.jpg",
        "Surya Hindu god": DEITY_ASSETS / "surya.jpg",
        "Vishnu Hindu god": DEITY_ASSETS / "vishnu.jpg",
        "Shani Hindu god": DEITY_ASSETS / "shani.jpg",
    }

    missing = [str(path) for path in required.values() if not path.exists()]
    if missing:
        raise RuntimeError(
            "Missing bundled HD deity artwork: " + ", ".join(missing)
        )

    # Validate that every image is readable before rendering begins.
    for name, path in required.items():
        try:
            ...
            raise RuntimeError(
                f"Bundled deity artwork is unreadable for {name}: {path}"
            ) from exc
        print(f"Using bundled HD deity artwork: {name} -> {path}")

    credits = [
        "Bundled devotional artwork used by Daily Astro Hindi Video",
        "All deity artwork is local; no external image download is required.",
    ]
    CREDITS.write_text("\n".join(credits) + "\n", encoding="utf-8")
    return required
...
def main():
    ...
    print("Deity art: bundled HD devotional artwork (1600x1600)")
    print("========================================")
